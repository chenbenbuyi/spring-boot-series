...
public class ActivitiTest {
     ...
     * 部署流程定义（即将定义的流程文件入库）
     * 注入插入以下几张表的数据：
     * act_re_deployment 流程部署表，每次部署都会新增一条部署记录。
     * 比如同一个请假记录流程，只需部署一次(但每个流程时可以多次部署的)，但是不同的人请假就会产生多条请假申请的流程定义（act_re_procdef）
    ...
    public void testQueryProcessDefinition() {
        ...
        List<ProcessDefinition> processDefinitions = processDefinitionQuery.processDefinitionKey("2").orderByProcessDefinitionVersion().desc().list();
     ...
     *
     * <p>
     * act_hi_actinst  流程执行历史节点表 如任务节点的开始时间、结束时间，不同于act_hi_taskinst，该表还会记录起止节点等其它信息，act_hi_taskinst只会记录任务的历史信息
    ...
    public void testStartProcess() {
        // 这里的key 为流程的ID
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("id_3");
...
//        // 根据流程标识key和负责人进行查询
//        List<Task> lists = taskQuery.processDefinitionKey("id_3").list();
//        for (Task task : lists) {
